...
class EEP(object):
    ...
    # def __load_xml(self):
    #     print(self.soup.find_all("number"))
    #     self.telegrams = {
    #         enocean.utils.from_hex_string(telegram['number']): {
    #             enocean.utils.from_hex_string(function['number']): {
    #                 enocean.utils.from_hex_string(type['number'], ): type
    #                 for type in function.find_all('type')
    #             }
    #             for function in telegram.find_all('func')
    #         }
    #         for telegram in self.soup.find_all('rorg')
    #     }
    ...
    def _get_value(self, source, bitarray):
        ...
        return {
            source.find('shortcut').string.strip(): {
                'description': source.find('description').string or "None",
                'unit': source.find('unit').string or "None",
    ...
    def _get_enum(self, source, bitarray):
        ...
        return {
            source.find('shortcut').string.strip(): {
                'description': source.find('description').string or "None",
                'unit': source.find('unit', ''),
                'value': value_desc.find_next('description').string or "None",
                'raw_value': raw_value,
            }
        }
    ...
    def find_profile(self, bitarray, eep_rorg, rorg_func, rorg_type, direction=None, command=None):
        ...
        if profile.find('case') is not None:
            Condit = profile.find_all('condition')
            for option in Condit:
                ...
                if cond is not None:
                    ...
                    break
            if Case is not None:
                return Case.find_all('datafield')
            else:
                self.logger.warn('Cannot find message contents condition!')
                return None
                
        # extract data description
        # the direction tag is optional
        if direction is None:
            return profile.find_all('datafield')
        
        return profile.find_all('datafield', {'direction': direction}, recursive=False)
