...
from enocean.protocol.constants import PACKET, RORG, PARSE_RESULT, DECRYPT_RESULT, DB0, DB2, DB3, DB4, DB6, SLF_INFO
...
class Packet(object):
    ...
    def parse_msg(buf):
        ...
        if packet_type == PACKET.RADIO_ERP1:
            Chained_packet = ChainedMSG()
            # Need to handle UTE Teach-in here, as it's a separate packet type...
            if data[0] == RORG.UTE:
                packet = UTETeachInPacket(packet_type, data, opt_data)
            # elif data[0] == RORG.CDM:
            #     packet = ChainedMSG(packet_type, data, opt_data)
            #     if packet.idx == 0:
            #         self.CDMrcv = True
            #     elif Chained_packet.CDM_receive == True
            #         Chained_packet.add_msg(packet)
            else:
...
class MSGChainer(object):
    ...
    def parse_CDM(self, packet):
        ...
        if self.msglist == [] and packet.idx > 0:
            self.clean_list
            return None
        
        if packet.idx == 0:
            self.remaining_size = packet.chain_len - (len(packet.data) - 10)
            self.chained_data.extend(packet.data[4:-5])
            print(self.chained_data)
            self.msglist.append(packet)
            return None
        elif packet.id == self.msglist[0].id:
        ...
        if self.remaining_size == 0:
            return VirtualPacket(PACKET.RADIO_ERP1,self.chained_data, packet.optional)
