...
class ChainedMSG(RadioPacket):
    ...
    @staticmethod
    def create_CDM(over_sized_packt):
        if len(over_sized_packt) <= 15 and over_sized_packt.destination != [0xFF,0xFF,0xFF,0xFF]:
            return over_sized_packt
        
        if over_sized_packt.rorg == RORG.SEC_ENCAPS:
            CDM_RORG = RORG.SEC_CDM
        else:
            CDM_RORG = RORG.CDM

        data = over_sized_packt.data[:-5]
        # 55:00:10:07:01:CE:31:F0:5F:C1:38:61:68:FF:8A:7F:C2:00:00:00:00:00:03:05:03:06:1B:FF:00:1B
...
class SECTeachInPacket(RadioPacket):
    ...
    def create_SECTI_chain(rorg=0x35, RLC=None, SLF=None, PSK=0, TYPE=0, INFO=0,sender=None, destination=None):
        ...
        for i in range(N-1):
            ...
            # print(enocean.utils.to_hex_string(sec_ti_list[i].build()))
