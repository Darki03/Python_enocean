...
class Packet(object):
    ...
    def decrypt(self, Key, RLC = None, SLF_TI = 0x00, Window_Size = None):
        ...
        if SLF_IN.RLC_TX == SLF_INFO.RLC_TX_YES:
            ...
            RLC = self.data[DATA_END:DATA_END+RLC_SIZE]
            CMAC = self.data[DATA_END+RLC_SIZE:-5]
        else:     
            CMAC = self.data[DATA_END:-5]
            Data_in = self.data[1:DATA_END]
            RLC = security.find_RLC(Key, self.data[:DATA_END], CMAC, RLC_Start, Window_Size)
        ...
        return Out_packet, DECRYPT_RESULT.OK
...
class RadioPacket(Packet):
    ...
    def create_raw(rorg, Raw_Data, direction=None, destination=None,
               sender=None):
        return Packet.create_raw(PACKET.RADIO_ERP1, rorg, Raw_Data, direction, destination, sender)
