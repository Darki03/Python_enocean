...
from enocean.protocol.constants import PACKET, RORG, SLF_INFO
...
def find_RLC(K, Data_CMAC, CMAC_in, RLC_in, WS):
    ...
    int_RLC_in = enocean.utils.combine_hex(RLC_in)
...
def encdec_tlgrm(Key, In_packet: Packet, RLC, SLF_TI):
    ...
    #print(In_packet.rorg)
    if In_packet.rorg == RORG.SEC_ENCAPS:
        Data_in = In_packet.data[1:-5-MAC_SIZE]
        Decrypt_flag = True
        #print(Decrypt_flag)
    else:
        Data_in = In_packet.data[:-5]

    #print(enocean.utils.to_hex_string(Data_in))

    # Encrypt data
    Data_in = VAES128(Key, Data_in, RLC)

    if Decrypt_flag == True: 
        Out_packet.data.extend(Data_in)
        Out_packet.data.extend(In_packet.data[-5:])
        Out_packet.optional.extend(In_packet.optional)
    else:
        #Out_packet.data.append(RORG.SEC_ENCAPS)
        Out_packet.rorg = RORG.SEC_ENCAPS
        Out_packet.data.append(Out_packet.rorg)
        Out_packet.data.extend(Data_in)
        Out_packet.data.extend(CMAC_calc(Key, Out_packet.data, RLC, MAC_SIZE))
        Out_packet.data.extend(In_packet.data[-5:])
        Out_packet.optional.extend(In_packet.optional)
